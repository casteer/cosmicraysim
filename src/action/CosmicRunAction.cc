...
void CosmicRunAction::EndOfRunAction(const G4Run* run)
{
  ...
  std::cout << "ACT: --> Final Trigger Rate (" << Analysis::Get()->GetNSavedEvents() << "): " << rate << " +/- " << err_rate << " s^-1" << std::endl;
  ...
  for(int i=0;i< (int) counts.size();i++){
    ...
    std::cout << "ACT: --> Final Trigger Counts (" << counts.at(i) << "): " << i << " \t " << rate << " +/- " << err_rate << " s^-1" << std::endl;
  }
